...
export default function SurveyParticipantPage() {
  ...
  const initialLocale = (search.get("lang") as Locale) || (typeof window !== "undefined" ? ((localStorage.getItem("locale") as Locale) || "en") : "en");
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        setSession(res);
        // hydrate current state (likes + participant comments)
        try {
          const state = await api.getResponseState(res.response_id);
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          <h1 className="text-2xl font-bold mb-2">{t(locale, "thankYou")}</h1>
          <p className="text-gray-500">{t(locale, "recorded")}</p>
        </div>
      </div>
    );
  }
            ...
            <div key={post.id} data-post-id={post.id} className="bg-white rounded-lg border shadow-sm">
              ...
              <div className="px-4 py-2 flex items-center justify-between text-sm text-gray-500">
                ...
                <span className="flex gap-4">
                  {post.show_comments && (
                    <span>
                      {(post.display_comments_count + post.comments.length + (participantComments[post.id]?.length || 0))} {t(locale, "comments")}
                    </span>
                  )}
              ...
              {/* Comments: Researcher + Participant */}
              {(post.comments.length > 0 || (participantComments[post.id]?.length || 0) > 0) && (
                <div className="border-t px-4 py-3 space-y-2">
                  ...
                  {(participantComments[post.id] || []).map((c) => (
                    <div key={`p-${c.id}`} className="flex gap-2 items-start">
                      <div className="w-7 h-7 bg-blue-200 rounded-full flex-shrink-0 flex items-center justify-center text-xs font-bold text-blue-700">U</div>
                      <div className="bg-blue-50 rounded-2xl px-3 py-2 text-sm flex-1">
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            setParticipantComments((prev) => ({
                              ...prev,
                              [post.id]: (prev[post.id] || []).map((pc) => pc.id === c.id ? { ...pc, text: val } : pc),
                            }));
                          }}
                          onBlur={async (e) => {
                            const val = e.target.value;
                            if (val.trim() && session) {
                              await api.updateParticipantComment(session.response_id, c.id, val);
                            }
                          }}
                        />
                      </div>
                      <button
                        ...
                        onClick={async () => {
                          if (!session) return;
                          await api.deleteParticipantComment(session.response_id, c.id);
                          setParticipantComments((prev) => ({
                            ...prev,
                            [post.id]: (prev[post.id] || []).filter((pc) => pc.id !== c.id),
                          }));
                        }}
                      >
                        Delete
